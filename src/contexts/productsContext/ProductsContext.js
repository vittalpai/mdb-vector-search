...
const ProductsContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const productsRes = await getAllProductsService('shirts');
        if (productsRes.status === 200) {
          dispatch({
            type: actionTypes.INITIALIZE_PRODUCTS,
            payload: productsRes.data.docs,
          });
        }
  ...
  const updateProducts = (searchText) => {
    setLoading(true);
    (async () => {
      try {
        const productsRes = await getAllProductsService(searchText);
        if (productsRes.status === 200) {
          dispatch({
            type: actionTypes.INITIALIZE_PRODUCTS,
            payload: productsRes.data.docs,
  ...
  const updateProductsUsingImage = (imageUrl) => {
    setLoading(true);
    (async () => {
      try {
        const productsRes = await getProductsUsingImageSearch(imageUrl);
        if (productsRes.status === 200) {
          dispatch({
            type: actionTypes.INITIALIZE_PRODUCTS,
            payload: productsRes.data.docs,
