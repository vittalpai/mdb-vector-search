...
const ProductListing = () => {
  ...
  const { loading, filters } = useProductsContext();
  const { searchText } = filters;
  const headingText = searchText ? `Search results for "${searchText}"` : "Discover Your Style!";
      ...
      {loading ? (
        ...
        <div>
          ...
          {productsList.length > 0 ? (
            <main className="relative grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5">
          ...
          <button
            className={` fixed bottom-20 bg-gray-800 right-2 p-2 rounded-full text-xl shadow-2xl transition-all delay-100 ease-in-out ${
